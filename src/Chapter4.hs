...
@
-}
-- It doesn't compile because the return type of fmap is (Maybe b) but x is of
-- type (Maybe a).
{- |
=⚔️= Task 2
...
Cons 2 (Cons 3 (Cons 4 (Cons 3 (Cons 4 (Cons 5 Empty)))))
-}
...
listConcat = go
  where
    go Empty = Empty
    go (Cons x xs) = x `listAppend` go xs
...
andM mx my = mx >>= \x -> if x then my else return False
...
-- NOTE: I'm not sure what you meant by reverse the tree
